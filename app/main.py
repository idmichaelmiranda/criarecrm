...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        n_fix_gl = sla_service.fix_go_live_sla_status(db)
        if n_fix_gl:
            print(f"[SLA] Startup: {n_fix_gl} implantacao(es) com Go Live e SLA normalizado")
...
def _migrate_sqlite():
    ...
    new_cols = [
        ...
        "ALTER TABLE implantacoes ADD COLUMN responsavel_id INTEGER REFERENCES usuarios(id)",
        "ALTER TABLE implantacoes ADD COLUMN data_go_live DATE",
...
def _migrate_postgres() -> None:
    ...
    new_cols = [
        ...
        "ALTER TABLE implantacoes ADD COLUMN IF NOT EXISTS responsavel_id INTEGER REFERENCES usuarios(id)",
        "ALTER TABLE implantacoes ADD COLUMN IF NOT EXISTS data_go_live DATE",
