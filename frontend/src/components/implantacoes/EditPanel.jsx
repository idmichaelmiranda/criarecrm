...
// Formulário de edição de Implantação (status, responsável, consultor,
// prioridade, data prevista, observações, conversão de dados). Compartilhado
// entre a tela de detalhe completo (dentro do modal EditPanel) e o painel
// rápido do Kanban (inline, sem modal) — mesmos campos/validação nos dois
// lugares, só muda quem desenha a moldura ao redor.
export function ImplantacaoEditForm({ impl, onCancel, onSaved, showHeader = true }) {
    ...
    <>
      {showHeader && (
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          ...
          <button onClick={onCancel} className="text-gray-400 hover:text-gray-600 w-8 h-8 flex items-center justify-center rounded-lg hover:bg-gray-100 text-lg">✕</button>
        </div>
      )}
      <div className="px-6 py-5 space-y-4">
        ...
        <div className="flex gap-3 pt-1">
          <button onClick={onCancel} className="flex-1 py-2 rounded-lg border border-gray-200 text-sm text-gray-600 hover:bg-gray-50 transition-colors">{t("common.cancel")}</button>
...
// Modal centralizado (usado na tela de detalhe completo) — só a moldura,
// o formulário em si é o ImplantacaoEditForm acima.
export function EditPanel({ impl, onClose, onSaved }) {
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm">
        <ImplantacaoEditForm impl={impl} onCancel={onClose} onSaved={onSaved} />
      </div>
    </div>
  );
}
