...
const REGIME_CFG = {
  "1": { cls: "bg-green-50  text-green-700  border-green-200",  dot: "bg-green-500"  },
  "2": { cls: "bg-blue-50   text-blue-700   border-blue-200",   dot: "bg-blue-500"   },
  "3": { cls: "bg-violet-50 text-violet-700 border-violet-200", dot: "bg-violet-500" },
};
      ...
      <td className="px-4 py-3.5">
        {ativo ? (
          <span className="inline-flex items-center gap-1.5 text-[11px] font-semibold px-2.5 py-1 rounded-full bg-green-50 text-green-700 border border-green-200">
            <span className="w-1.5 h-1.5 rounded-full bg-green-500" /> {t("table.ativo")}
          </span>
        ) : (
          <span className="inline-flex items-center gap-1.5 text-[11px] font-semibold px-2.5 py-1 rounded-full bg-gray-100 text-gray-500 border border-gray-200">
            <span className="w-1.5 h-1.5 rounded-full bg-gray-400" /> {t("table.inativo")}
          </span>
        )}
      </td>
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-5">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            {loading ? t("subtitleLoading") : t("subtitle", { count: clientes.length })}
          </p>
        </div>
      ...
      {!loading && !error && total > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 mb-5">
          ...
          <StatCard
            ...
            label={t("stats.lucroPresReal")}
            sub={t("stats.lucroPresRealSub", { presumido, real })}
            icon={
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="py-12 text-center">
            ...
            <button onClick={refetch} className="text-xs text-orange-600 mt-2 hover:underline">{t("error.retry")}</button>
          </div>
        ) : (
          <>
            {/* Tabs */}
            <div className="flex items-center justify-between px-5 pt-4 pb-0 border-b border-gray-100">
              <div className="flex gap-0">
                {TABS.map((tabKey) => {
                  const count = tabKey === "todos" ? totalCount : tabKey === "ativos" ? ativosCount : inativosCount;
              ...
              {search && (
                <p className="text-xs text-gray-400 pb-2">
                  {t("search.resultCount", { count: filtered.length, term: search })}
                </p>
              )}
            </div>
              ...
              <>
                      ...
                      <tr className="border-b border-gray-100 text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wider bg-gray-50/50">
                        ...
                        <th className="px-4 py-3">
                          <button onClick={() => toggleSort("name")} className="hover:text-gray-600 transition-colors flex items-center gap-1">
                            {t("table.empresa")} <SortIcon active={sortField === "name"} dir={sortDir} />
                          </button>
                        </th>
                        <th className="px-4 py-3 hidden sm:table-cell">{t("table.cnpj")}</th>
                        <th className="px-4 py-3 hidden md:table-cell">{t("table.cidadeUf")}</th>
                        <th className="px-4 py-3 hidden lg:table-cell">{t("table.regime")}</th>
                        ...
                        <th className="px-4 py-3 hidden sm:table-cell">
                          <button onClick={() => toggleSort("updated")} className="hover:text-gray-600 transition-colors flex items-center gap-1">
                            {t("table.atualizado")} <SortIcon active={sortField === "updated"} dir={sortDir} />
                          </button>
                        </th>
                ...
                <div className="px-5 py-3 border-t border-gray-100 bg-gray-50/40 flex items-center justify-between">
                  <p className="text-xs text-gray-400">
                    {t("footer.mostrando")} <span className="font-semibold text-gray-600">{sorted.length}</span> {t("footer.de")}{" "}
                    <span className="font-semibold text-gray-600">{clientes.length}</span> {t("footer.clientes")}
                    {tab !== "todos" && (
                      <span className="ml-1">
                        {t("footer.filtro")} <span className="font-medium text-orange-500">{t(`tabs.${tab}`)}</span>
                        <button onClick={() => setTab("todos")} className="ml-1 text-gray-400 hover:text-gray-600 transition-colors">{t("footer.limpar")}</button>
                      </span>
                    )}
                    {search && (
                      <span className="ml-1">
                        {t("footer.busca")} <span className="font-medium text-orange-500">"{search}"</span>
                        <button onClick={() => setSearch("")} className="ml-1 text-gray-400 hover:text-gray-600 transition-colors">{t("footer.limpar")}</button>
                      </span>
                    )}
                  </p>
                  <div className="flex items-center gap-2">
                    {simples > 0 && (
                      <span className="inline-flex items-center gap-1 text-[10px] text-gray-400">
                        <span className="w-2 h-2 rounded-full bg-green-400" />{t("footer.legendSimples", { count: simples })}
                      </span>
                    )}
                    {presumido > 0 && (
                      <span className="inline-flex items-center gap-1 text-[10px] text-gray-400">
                        <span className="w-2 h-2 rounded-full bg-blue-400" />{t("footer.legendPresumido", { count: presumido })}
                      </span>
                    )}
                    {real > 0 && (
                      <span className="inline-flex items-center gap-1 text-[10px] text-gray-400">
                        <span className="w-2 h-2 rounded-full bg-violet-400" />{t("footer.legendReal", { count: real })}
                      </span>
                    )}
                  </div>
                </div>
