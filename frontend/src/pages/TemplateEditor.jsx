...
function TarefaRow({ tarefa, index, total, onUpdate, onDelete, onMoveUp, onMoveDown }) {
  ...
  function updateSubitem(idx, changes) {
    const subitens = [...(tarefa.subitens || [])];
    subitens[idx] = { ...subitens[idx], ...changes };
    onUpdate({ ...tarefa, subitens });
  }
        ...
        <div className="ml-8 mt-1 mb-1 rounded-lg border border-gray-100 bg-gray-50/60 overflow-hidden">
          ...
          {(tarefa.subitens || []).map((sub, si) => (
            <div key={sub._key ?? sub.id}
              className="flex items-center gap-2 px-3 py-1.5 border-b border-gray-100 last:border-b-0 group/sub hover:bg-white transition-colors">
              ...
              <button
                ...
                className="opacity-0 group-hover/sub:opacity-100 w-5 h-5 flex items-center justify-center rounded text-gray-300 hover:text-red-400 hover:bg-red-50 transition-all shrink-0"
              >
                <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                </svg>
          ...
          <button
            type="button"
            onClick={addSubitem}
            className="w-full flex items-center gap-1.5 px-3 py-1.5 text-[11px] text-gray-400 hover:text-orange-500 hover:bg-orange-50/40 transition-colors"
          >
            <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5}>
