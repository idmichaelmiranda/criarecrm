...
export function AprovarModal({ sol, onClose, onApproved }) {
  ...
  // step: "form" | "confirmar_cliente" | "confirmar_email"
  const [step, setStep]                         = useState("form");
  const [clienteExistente, setClienteExistente] = useState(null);
  const [clienteEmailExistente, setClienteEmailExistente] = useState(null);
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      // 404 = CNPJ livre → verifica e-mail antes de aprovar direto
      await checarEmailEAprovar();
    } finally {
      setChecking(false);
    }
  }

  async function checarEmailEAprovar() {
    try {
      const { data: clienteEmail } = await clientesApi.buscarPorEmail(sol.email);
      // Encontrou um cliente com esse e-mail — só é conflito se for CNPJ diferente
      // (lojas do mesmo grupo podem compartilhar o e-mail de contato)
      if (clienteEmail.cnpj !== sol.cnpj) {
        setClienteEmailExistente(clienteEmail);
        setStep("confirmar_email");
        return;
      }
    } catch {
      // 404 = e-mail livre
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl flex flex-col" style={{ maxHeight: "90vh" }}>
        ...
        {/* ── Passo 3: e-mail já usado por outro cliente, pede confirmação ── */}
        {step === "confirmar_email" && clienteEmailExistente && (
          <div className="px-6 py-5 space-y-4">
            {/* Alerta */}
            <div className="flex gap-3 p-3 rounded-xl bg-amber-50 border border-amber-200">
              <svg className="w-5 h-5 text-amber-500 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              ...
              <div>
                <p className="text-sm font-semibold text-amber-800">{t("approveModal.emailExistsTitle")}</p>
                <p className="text-xs text-amber-700 mt-0.5 leading-relaxed">
                  {t("approveModal.emailExistsDesc")}
                </p>
              </div>
            </div>

            {/* Card do cliente que já usa esse e-mail */}
            <div className="rounded-xl border border-gray-200 bg-gray-50 divide-y divide-gray-100">
              <div className="px-4 py-3">
                <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest mb-0.5">{t("approveModal.existingClientLabel")}</p>
                <p className="text-sm font-semibold text-gray-900">{clienteEmailExistente.razao_social}</p>
                {clienteEmailExistente.nome_fantasia && (
                  <p className="text-xs text-gray-500">{clienteEmailExistente.nome_fantasia}</p>
                )}
              </div>
              <div className="px-4 py-3 grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest mb-0.5">{t("approveModal.cnpjLabel")}</p>
                  <p className="text-xs font-mono text-gray-700">{clienteEmailExistente.cnpj}</p>
                </div>
                <div>
                  <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest mb-0.5">{t("approveModal.emailLabel")}</p>
                  <p className="text-xs text-gray-700 truncate">{clienteEmailExistente.email}</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex gap-3 pt-1">
              <button type="button" onClick={onClose}
                className="flex-1 py-2 rounded-lg border border-gray-200 text-sm text-gray-600 hover:bg-gray-50 transition-colors">
                {t("approveModal.emailExistsFix")}
              </button>
              <button type="button" onClick={submeterAprovacao} disabled={saving}
                className="flex-1 py-2 rounded-lg text-sm font-semibold text-white bg-green-600 hover:bg-green-700 disabled:opacity-50 transition-colors flex items-center justify-center gap-2">
                {saving ? (
                  <><span className="w-3.5 h-3.5 rounded-full border-2 border-white/30 border-t-white animate-spin" /> {t("approveModal.approving")}</>
                ) : t("approveModal.emailExistsConfirm")}
              </button>
            </div>
          </div>
        )}
