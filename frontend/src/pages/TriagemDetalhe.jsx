  ...
  return (
    <div className="flex items-start mt-4">
      {[0, 1, 2].map((i) => {
        const dt = stepDate(i);
        return (
          // items-start (não items-center): a linha se ancora a uma distância fixa do
          // topo (mt-2.5, alinhado ao centro do círculo de 20px) em vez de se recentralizar
          // com base na altura total da coluna — que varia conforme a data aparece ou não.
          <div key={i} className={`flex items-start ${i < 2 ? "flex-1" : ""}`}>
            ...
            {i < 2 && <div className={`h-0.5 flex-1 mx-1.5 rounded-full mt-2.5 ${i < idx ? "bg-emerald-500" : "bg-white/10"}`} />}
          </div>
        );
      })}
    </div>
